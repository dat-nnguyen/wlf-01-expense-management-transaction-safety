...
class IntentPlanner:
    ...
    def plan(user_message: str) -> ExecutionPlan:
        ...
        if matches_any([
            ...
            "am i safe", "an toàn chứ", "an toan chu", "co bi hack khong", "có bị hack không",
            "an toan",
        ]):
        ...
        # 2. Specific $9.99 or Transaction Detail & Email Match Inquiry (Top Priority)
        if "$9.99" in msg or "9.99" in msg or "9,99" in msg or "khoản 9.99" in msg_norm:
            return ExecutionPlan(
        ...
        # 3. Transaction Authenticity & Scam Verification (Screenshot Verification)
        if matches_any([
        ...
        # 4. Check Disallowed Mutating Actions (Imperative commands - MUST REJECT)
        if (
            re.search(r"(?:hãy\s+|tự\s+|vui lòng\s+|hay\s+|tu\s+|vui long\s+)?(?:huỷ|hủy|huy|dừng|dung|ngắt|ngat|cancel|unsubscribe)\s+(?:subscription|gói|goi|dịch vụ|dich vu|membership|netflix|adobe|spotify|mấy gói)", msg_norm, re.IGNORECASE)
            or re.search(r"^(?:hãy\s+|vui lòng\s+|tự\s+|hộ\s+|hay\s+|vui long\s+|tu\s+|ho\s+)?(?:chuyển|chuyen|bắn|ban|gửi|gui|rút|rut|nạp|nap|pay|transfer|send|wire)\s+\$?\d+", msg_norm, re.IGNORECASE)
        ...
        # 5. Email Report Dispatch Request (HITL User Request)
        if matches_any([
            "gửi báo cáo", "gui bao cao", "vào email của tôi", "vao email cua toi",
            "send report to my email", "email báo cáo", "gửi báo cáo cho tôi", "gui bao cao cho toi",
            "gui bao cao vao email", "gửi vào email",
        ]):
            return ExecutionPlan(
        ...
        # 6. 3-Way Reconciliation Inquiries
        if matches_any([
            ...
            "3-way", "chua len card", "chua sang the", "rời tk",
        ]):
            return ExecutionPlan(
        ...
        # 7. Subscriptions & Price Hike Inquiry (Check before general email inquiry)
        if matches_any([
            ...
            "gói đăng ký", "goi dang ky", "gói định kỳ", "goi dinh ky", "gói tháng", "goi thang",
        ...
        if matches_any([
            ...
            "quẹt 2 lần", "quet 2 lan", "duplicate", "bị trừ đúp", "bi tru dup", "quẹt đúp", "quet dup",
            "tính hai lần", "tinh hai lan", "phí kép", "phi kep",
        ]):
            return ExecutionPlan(
        ...
        # 10. Specific Top 3 Expenses Inquiry
        if matches_any([
